...
- HyperVM 가상머신에 k8s를 설치하여, 외부 NFS 서버를 dynamic provisioning 하는 방법입니다.
...
## Prerequisites
1. HyperVM 가상머신에 k8s 클러스터가 설치 되어야 합니다. [[k8s 설치 가이드](https://github.com/jwmin401/kubernetes-installation.git)]
2. 본 가이드에서는 이미 NFS 서버가 존재하며 NFS provisioner를 배포할 k8s 환경과 통신이 가능함을 가정합니다.[[NFS 서버 설치 가이드](https://github.com/jwmin401/nfs-server.git)]
3. k8s 클러스터의 모든 노드에는 `nfs-utils` 패키지 설치가 필요합니다.
...
5. Cri-O 1.19.2 Version이 설치 되어 있어야 합니다.(Docker X)
...
``` shell
$ sudo crictl pull gcr.io/k8s-staging-sig-storage/nfs-subdir-external-provisioner:${NFS_PROVISIONER_VERSION}
image를 tar로 압축 할 때 필요(공용망에서는 필요 x)
$ sudo crictl image export gcr.io/k8s-staging-sig-storage/nfs-subdir-external-provisioner:${NFS_PROVISIONER_VERSION} > nfs_${NFS_PROVISIONER_VERSION}.tar
```
...
``` shell
$ NS={배포할 namespace name} (ex) nfs-server)
$ NAMESPACE=${NS:-nfs}
